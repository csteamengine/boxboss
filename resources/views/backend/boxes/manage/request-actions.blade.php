<div class="btn-group btn-group-sm" role="group" aria-label="@lang('labels.backend.boxes.actions')">
    ...
    @if($logged_in_user->can('edit', $box))
        <a href="{{ route('admin.requests.accept', ['box' => $box, 'id' => $request]) }}" class="btn btn-success" data-toggle="tooltip"
...
{{--    TODO find out how to change this text for the confirm box--}}
    @if($logged_in_user->can('edit', $box))
        <a href="{{ route('admin.requests.decline', ['box' => $box, 'id' => $request]) }}" class="btn btn-danger" data-toggle="tooltip"
           data-placement="top"
           title="Decline"
           data-method="get"
           data-trans-button-cancel="@lang('buttons.general.cancel')"
