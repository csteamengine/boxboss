<div class="btn-group btn-group-sm" role="group" aria-label="@lang('labels.backend.boxes.actions')">
    @if($logged_in_user->can('view', $box))
        <a href="{{ route('admin.boxes.view', $box) }}" class="btn btn-outline-primary" data-toggle="tooltip"
           data-placement="top" title="Manage">
            <i class="fas fa-tasks"></i>
        </a>
    @endif
    @if($logged_in_user->can('edit', $box))
        <a href="{{ route('admin.boxes.edit', $box) }}" class="btn btn-outline-success" data-toggle="tooltip"
